...
public class RainStormLeader {
    ...
    private RainStormLeader(int numStages, int numTasks, boolean exactlyOnce, boolean autoscaleEnabled, int inputRate, int lw, int hw) {
    ...
    private void launchAllWorkerTasks() throws Exception {
        for (TaskInfo t : tasks.values()) {
            ...
            StartWorkerTaskRequest req = new StartWorkerTaskRequest(t.globalTaskId, chooseOperator(t.stageIdx), isFinal, operatorArgs(t.stageIdx));
